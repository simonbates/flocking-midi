...
fluid.defaults("flock.midi.demos.portsView", {
    ...
    markup: {
        ...
        port: "<tr><td>%id</td><td class='fl-midi-portsView-name'>%manufacturer %name</td><td>%state</td><td>%connection</td></tr>"
    },
...
flock.midi.demos.portsView.renderPort = function (port, sectionHeader, options) {
    var rendered = fluid.stringTemplate(options.markup.port, port);
    sectionHeader.after(rendered);
};

flock.midi.demos.portsView.renderPorts = function (ports, sectionheader, options) {
    fluid.each(ports, function (port) {
        flock.midi.demos.portsView.renderPort(port, sectionheader, options);
    });
};
